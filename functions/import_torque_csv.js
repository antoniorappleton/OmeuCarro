...
function mapRowToReading(row) {
    ...
    // Helper to find key containing string (case insensitive) to avoid encoding issues
    const findKey = (part) => Object.keys(row).find(k => k.toLowerCase().includes(part.toLowerCase()));

    const odoKey = findKey('Odometer') || 'Odometer(from ECU)(km)';
    const speedKey = 'Speed (OBD)(km/h)'; // User specific
    const rpmKey = findKey('RPM');
    const fuelKey = findKey('Fuel Level');
    const coolantKey = findKey('Coolant');
    const latKey = 'Latitude';
    const lonKey = 'Longitude';

    const parsed = {
        speed: getNum(speedKey),
        rpm: getNum(rpmKey),
        odometer: getNum(odoKey),
        fuelLevel: getNum(fuelKey),
        coolant: getNum(coolantKey),
        engineLoad: getNum(findKey('Engine Load')),
        location: null
    };
...
exports.importTorqueCsv = onRequest(async (req, res) => {
  ...
  try {
    ...
    for (const chunk of chunks) {
        ...
        const collectionRef = db.collection("veiculos").doc(VEICULO_ID).collection("leiturasObd");
    ...
    if (latest && latest.parsed) {
        ...
        if (latest.parsed.odometer > 0) updatePayload.odometroAtual = latest.parsed.odometer;
        if (latest.parsed.fuelLevel > 0) updatePayload.nivelCombustivel = latest.parsed.fuelLevel;
        if (latest.parsed.location) updatePayload.localizacao = latest.parsed.location;
        
        // Always update the 'lastObdUpdate' timestamp
        updatePayload.ultimaLeituraObd = admin.firestore.FieldValue.serverTimestamp();
