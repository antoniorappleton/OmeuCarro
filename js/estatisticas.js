...
async function carregarEstatisticas(filterVeiculoId = null) {
  ...
  try {
    ...
    // FILTRO
    if (filterVeiculoId) {
        abastecimentos = abastecimentos.filter(a => a.veiculoId === filterVeiculoId);
    }
    ...
    Object.values(registersByVehicle).forEach(listaVeiculo => {
        ...
        listaVeiculo.forEach(abs => {
             ...
             if (d && !isNaN(d.getTime())) {
                ...
                mesKey = `${ano}-${mes}`;
                if (!porMes[mesKey]) porMes[mesKey] = { gasto: 0, litros: 0, kmPercorridos: 0 };
             ...
             // Posto
             const posto = abs.posto && abs.posto.trim() ? abs.posto.trim() : "Sem posto";
             if (!porPosto[posto]) porPosto[posto] = { visitas: 0, totalGasto: 0, litros: 0 };
    ...
    Object.values(porMes).forEach(m => totalKmPercorridos += m.kmPercorridos);

    let eficiencia = 0;
    if (totalLitros > 0 && totalKmPercorridos > 0) {
        eficiencia = totalKmPercorridos / totalLitros; // km/L
    }
    ...
    if (eficienciaEl) eficienciaEl.textContent = `${eficiencia.toFixed(1)} km/L`;
    ...
    // 1. GASTOS
    createOrUpdateChart("chart-gastos-mensais", "bar", {
         labels: labelsMeses,
         datasets: [{ label: "Total (€)", data: dataGasto, backgroundColor: "#0c8c78", borderRadius: 4 }]
    }, { responsive: true, maintainAspectRatio: false, plugins: { legend: { display: false } } });

    // 2. LITROS
    createOrUpdateChart("chart-litros-mensais", "bar", {
        labels: labelsMeses,
        datasets: [{ label: "Litros", data: dataLitros, backgroundColor: "#3b82f6", borderRadius: 4 }]
    }, { responsive: true, maintainAspectRatio: false, plugins: { legend: { display: false } } });
    ...
    createOrUpdateChart("chart-preco", "line", {
        ...
        datasets: [{ 
            ...
            tension: 0.1, fill: true, pointRadius: 1 
        }]
    }, { responsive: true, maintainAspectRatio: false, plugins: { legend: { display: false } } });

    // 4. TIPOS
    createOrUpdateChart("chart-tipos", "doughnut", {
    ...
    createOrUpdateChart("chart-postos", "bar", {
        labels: postosSorted.map(p => p.nome),
        datasets: [{ label: "Visitas", data: postosSorted.map(p => p.visitas), backgroundColor: "#0c8c78", borderRadius: 4 }]
    }, { indexAxis: "y", responsive: true, maintainAspectRatio: false, plugins: { legend: { display: false } } });

    // 6. KM MENSAIS
    createOrUpdateChart("chart-km-mensais", "bar", {
        labels: labelsMeses,
        datasets: [{ label: "Km", data: dataKm, backgroundColor: "#f59e0b", borderRadius: 4 }]
    }, { responsive: true, maintainAspectRatio: false, plugins: { legend: { display: false } } });

    // 7. CUSTO/100KM
    createOrUpdateChart("chart-custo-100km", "line", {
        labels: labelsMeses,
        datasets: [{ 
            label: "€/100km", 
            data: dataCusto100, 
    ...
    createOrUpdateChart("chart-postos-baratos", "bar", {
        labels: postosBaratos.map(p => `${p.nome} (€${p.precoMedio.toFixed(3)})`),
        datasets: [{ label:"€/L", data: postosBaratos.map(p => p.precoMedio), backgroundColor: "#10b981", borderRadius: 4 }]
    }, { 
        indexAxis: "y", 
        responsive: true, 
