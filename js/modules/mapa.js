...
if (window.__L100_MAP_INIT__) {
  ...
  async function fetchAddressSuggestions(query) {
    ...
    try {
      const url = `https://nominatim.openstreetmap.org/search?format=json&q=${encodeURIComponent(query)}&limit=5&accept-language=pt&addressdetails=1`;
      const res = await fetch(url);
      if (!res.ok) return [];
    ...
    state.favorites.forEach((fav) => {
      ...
      const iconHtml = `
                    <div class="pin-shape pin-rotate-base">
                       <svg class="icon pin-icon-rotate"><use href="../assets/icons/icons-unified.svg#${iconName}"></use></svg>
                    </div>
  ...
  function renderSearchResults(term) {
    ...
    matches.forEach((fav) => {
      ...
      item.innerHTML = `
                     ...
                     <div><svg class="icon icon-result"><use href="../assets/icons/icons-unified.svg#icon-car"></use></svg></div>
                 `;
  ...
  function renderFormCategoryChips() {
    ...
    state.availableCategories.forEach((cat) => {
      ...
      btn.innerHTML = `<svg class="icon"><use href="../assets/icons/icons-unified.svg#${icon}"></use></svg> ${cat}`;
    ...
    addBtn.innerHTML = `<svg class="icon"><use href="../assets/icons/icons-unified.svg#icon-plus"></use></svg> Novo`;
    ...
    addBtn.onclick = (e) => {
      ...
      addBtn.innerHTML = `
            ...
            <div class="chip-actions">
                <button id="btn-confirm-cat" class="icon-btn-xs"><svg class="icon"><use href="../assets/icons/icons-unified.svg#icon-check"></use></svg></button>
                <button id="btn-cancel-cat" class="icon-btn-xs"><svg class="icon"><use href="../assets/icons/icons-unified.svg#icon-close"></use></svg></button>
            </div>
        `;
  ...
  function openForm(fav = null) {
    ...
    // Clear suggestions & existing dataset coords
    els.inpAddr.dataset.lat = "";
    els.inpAddr.dataset.lng = "";
    document.getElementById("addr-suggestions").innerHTML = "";
    document.getElementById("addr-suggestions").classList.add("hidden");
  }

  // Address Autocomplete Handlers
  els.inpAddr.addEventListener("input", (e) => {
    // Clear dataset on manual edit
    els.inpAddr.dataset.lat = "";
    els.inpAddr.dataset.lng = "";
    ...
    autocompleteTimer = setTimeout(async () => {
      const suggestionsContainer = document.getElementById("addr-suggestions");
      const spinner = document.getElementById("geocoding-spinner");
      const icon = document.getElementById("geocoding-icon");
      ...
      if (results.length > 0) {
        ...
        results.forEach(res => {
          ...
          item.innerHTML = `
            ...
            <span class="suggestion-addr">${subtitle}</span>
          `;
          
          item.onclick = () => {
            els.inpAddr.value = res.display_name;
            suggestionsContainer.classList.add("hidden");
            // Store coordinates safely to avoid re-geocoding on save
            els.inpAddr.dataset.lat = res.lat;
            els.inpAddr.dataset.lng = res.lon;
            
            // Smart Name Suggestion update based on selected address if name is still empty
            if (!els.inpName.value.trim()) {
               els.inpName.value = title;
            }
          };
  ...
  document
    ...
    .addEventListener("click", (e) => {
      ...
      if (chip && chip.hasAttribute("data-value")) {
        ...
        // Smart Suggestion: If name is empty or already a category name, update it
        const currentName = els.inpName.value.trim();
        const categories = ["Casa", "Trabalho", "Outro"];
        if (!currentName || categories.includes(currentName)) {
           els.inpName.value = val !== "Outro" ? val : "";
        }
      }
    });
  ...
  document
    ...
    .addEventListener("click", async () => {
      ...
      try {
        ...
        // Priority 1: Check if we have cached coords from autocomplete
        if (els.inpAddr.dataset.lat && els.inpAddr.dataset.lng) {
            lat = parseFloat(els.inpAddr.dataset.lat);
            lng = parseFloat(els.inpAddr.dataset.lng);
            needsGeocoding = false;
        } 
        // Priority 2: Check if it's the same as existing and we have coords
        else if (state.isEdit && state.selected) {
          if (
