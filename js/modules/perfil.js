...
auth.onAuthStateChanged(async (user) => {
  ...
  if (displayNameHeader)
    displayNameHeader.textContent = user.displayName || "Utilizador";
  ...
  try {
    ...
    if (settings) {
      // Regional
      if (langSelect)
        langSelect.value = settings.idioma || userProfile?.idioma || "pt";
      if (currencySelect)
        currencySelect.value = settings.moeda || userProfile?.moeda || "EUR";
      if (distUnitSelect)
        distUnitSelect.value = settings.unidadeDistancia || "km";
      if (consUnitSelect)
        consUnitSelect.value =
          settings.unidadeConsumo || userProfile?.unidadeConsumo || "L/100km";

      // Refueling
      if (fuelDefaultSelect)
        fuelDefaultSelect.value = settings.combustivelPadrao || "";
      if (fillFullToggle)
        fillFullToggle.checked =
          settings.abastecimentoCompletoDefault !== false;
      if (validateOdoToggle)
        validateOdoToggle.checked = settings.validarOdometro !== false;

      // Vehicles
      if (showInactiveToggle)
        showInactiveToggle.checked = !!settings.mostrarInativos;

      // Dashboard
      if (dashPeriodSelect)
        dashPeriodSelect.value = settings.dashboardPeriodo || "mes";
      if (chartTypeSelect)
        chartTypeSelect.value = settings.tipoGrafico || "bar";

      // KPIs
      if (settings.dashboardKpis) {
        if (kpiGastos)
          kpiGastos.checked = settings.dashboardKpis.gastos !== false;
        if (kpiConsumos)
          kpiConsumos.checked = settings.dashboardKpis.consumos !== false;
        if (kpiDistancias)
          kpiDistancias.checked = settings.dashboardKpis.distancias !== false;
      }

      // Alerts
      if (notifActiveToggle) {
        notifActiveToggle.checked = settings.notificacoesAtivas !== false;

        // Listener para pedir permissão se ligar o switch
        notifActiveToggle.addEventListener("change", async (e) => {
          if (e.target.checked) {
            try {
              if (window.requestNotificationPermissionAndSaveToken) {
                await window.requestNotificationPermissionAndSaveToken();
                alert("Notificações ativadas com sucesso! ðŸ””");
              }
            } catch (err) {
  ...
  settingsForm.addEventListener("submit", async (e) => {
    ...
    try {
      const payload = {
        ...
        abastecimentoCompletoDefault: fillFullToggle
          ? fillFullToggle.checked
          : true,
        ...
        mostrarInativos: showInactiveToggle
          ? showInactiveToggle.checked
          : false,
        ...
        // Alerts
        notificacoesAtivas: notifActiveToggle
          ? notifActiveToggle.checked
          : true,
        alertaAntecedencia: alertDaysSelect ? alertDaysSelect.value : "15",

        // Data (just UI preferences like includeDocs)
        incluirDocsExportacao: includeDocsToggle
          ? includeDocsToggle.checked
          : false,
      };
      ...
      alert("Definições guardadas com sucesso! âœ…");
...
if (btnResetPassword) {
  btnResetPassword.addEventListener("click", async () => {
    ...
    if (
      confirm(
        `Enviar email de redefinição de palavra-passe para ${user.email}?`,
      )
    ) {
      try {
...
if (btnExportCsv) {
  btnExportCsv.addEventListener("click", async () => {
    ...
    try {
      ...
      link.setAttribute(
        "download",
        `l100_export_${new Date().toISOString().slice(0, 10)}.csv`,
      );
...
if (btnDeleteAccount) {
  btnDeleteAccount.addEventListener("click", async () => {
    if (
      confirm(
        "ATENí‡íƒO: A sua conta e todos os dados serão eliminados permanentemente. Tem a certeza absoluto?",
      )
    ) {
      const user = auth.currentUser;
      try {
        ...
        if (err.code === "auth/requires-recent-login") {
          alert(
            "Por motivos de segurança, faça login novamente e tente apagar a conta.",
          );
          await auth.signOut();
          window.location.href = "auth.html";
        } else {
...
function updateAppStatus() {
  ...
  if (appLastSync) {
    const now = new Date();
    appLastSync.textContent = now.toLocaleTimeString([], {
      hour: "2-digit",
      minute: "2-digit",
    });
  }
