...
firebase.auth().onAuthStateChanged(async (user) => {
  ...
  // Set Date
  const now = new Date();
  dateDisplay.textContent = `Data: ${now.toLocaleDateString('pt-PT')} • ${now.toLocaleTimeString('pt-PT', {hour: '2-digit', minute:'2-digit'})}`;
  ...
  try {
    ...
    vehicleSelect.onchange = async () => {
      ...
      if (selectedVehicle) {
        ...
        const vLogs = allLogs.filter(l => l.veiculoId === vid).sort((a,b) => new Date(b.data) - new Date(a.data));
        
        const lastLog = vLogs[0];
        const lastKm = lastLog ? lastLog.km : (selectedVehicle.odometroInicial || 0);
